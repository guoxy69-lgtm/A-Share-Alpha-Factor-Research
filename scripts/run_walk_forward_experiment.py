...
from mfe5210_alpha.experiments.walk_forward import run_walk_forward_experiment
...
if __name__ == "__main__":
    ...
    result = run_walk_forward_experiment(panel, signal_diagnostics, cfg.factor_root, cfg)
    output_dir = cfg.output_root / "walk_forward"
    output_dir.mkdir(parents=True, exist_ok=True)
    result["performance"].to_csv(output_dir / "oos_daily_returns.csv", index=False)
    result["positions"].to_csv(output_dir / "oos_positions.csv", index=False)
    ...
    result["weights"].to_csv(output_dir / "rolling_factor_weights.csv", index=False)
    print(output_dir / "oos_daily_returns.csv")
